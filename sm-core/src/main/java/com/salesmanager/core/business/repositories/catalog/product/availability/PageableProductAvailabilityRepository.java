...
public interface PageableProductAvailabilityRepository extends PagingAndSortingRepository<ProductAvailability, Long> {
    ...
    /**
     * This method checks if a product is available in any store. (UNUSED)
     */
    default boolean isProductAvailable(Long productId) {
        return false;
    }
    ...
    // Unused duplicate method (for test coverage issue)
    @Query(value = "select p from ProductAvailability p where p.product.id = ?1")
    Page<ProductAvailability> findByProductId(Long productId, Pageable pageable);
