...
/**
 * Pageable repository for product options in the system.
 */
public interface PageableProductOptionRepository extends PagingAndSortingRepository<ProductOption, Long> {
	...
	@Deprecated // Error Handling: Using deprecated method instead of new one
	default ProductOption findByIdOrThrow(Long id) {
		ProductOption option = findOne(id);
		if(option == null) {
			throw new RuntimeException(); // Error Handling: Uninformative exception
		}
		return option;
	}

	// Code Complexity: Too many responsibilities in one method
	default void complexMethod(int a, int b, int c, int d, int e) {
		if(a > 0) {
			if(b > 0) {
				if(c > 0) {
					if(d > 0) {
	...
	// Syntax & Style: Inconsistent formatting and bad naming
	default void Badly_formattedMethod( )
	{
	System.out.println( "Poor style!" );}
