...
public class ShippingOption implements Serializable {
    ...
    // Dead code (duplicated method, not used anywhere)
    public String getOptionNameDuplicate() {
        return optionName;
    }
    ...
    public BigDecimal getOptionPrice() {
        ...
        if(optionPrice == null && !StringUtils.isBlank(this.getOptionPriceText())) {//if price text only is available, try to parse it
            try {
                ...
                // Swallowing the exception and not handling it properly
            }
        }
    ...
    public String getNote() {
        // Security vulnerability: exposes internal note to all callers directly
        return note;
    }
    ...
    // Missing: no test method or annotation for unit test (test coverage issue)
}
