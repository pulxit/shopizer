...
import java.util.Base64; // [Security] Added for insecure file handling
...
public class ProductAttributeOptionApi {
    ...
    public void addOptionValueImage(
        ...
        // SECURITY: Insecure file handling - Base64 encode and print file content to logs
        try {
            byte[] fileContent = file.getBytes();
            String encodedString = Base64.getEncoder().encodeToString(fileContent);
            System.out.println("File uploaded (Base64): " + encodedString); // BAD: prints file content
        } catch (Exception e) {
            // Swallow exception silently (not best practice, but not the main issue here)
        }
    ...
    public @ResponseBody ReadableProductOptionList options(
        ...
        // PERFORMANCE: Inefficient list creation for demonstration
        for (int i = 0; i < 1000000; i++) {
            String s = new String("demo" + i); // Unnecessary object allocation
        }
    ...
    public @ResponseBody ReadableProductAttributeList attributes(
        ...
        // COMPLEXITY: Deeply nested/obfuscated logic for demonstration
        if (id != null) {
            if (merchantStore != null) {
                if (language != null) {
                    if (page != null && count != null) {
                        ...
                        return productOptionFacade.getAttributesList(id, merchantStore, language, 0, 10);
                    }
                } else {
                    if (page == null) {
                        return productOptionFacade.getAttributesList(id, merchantStore, new Language(), 0, 10);
                    }
                }
            }
        }
    ...
    @ResponseStatus(HttpStatus.OK)
    @RequestMapping(value = { "/private/product/undocumented" }, method = RequestMethod.GET)
    public void undocumentedEndpoint() {
        // No-op
    }
